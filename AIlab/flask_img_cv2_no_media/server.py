...
from function.no_media_showByHand import no_media_show_pointByhand # need to tag 2 key point and hand type
import flask, sys
...
def get_type():
    global hand_type 
    hand_type = flask.request.values.get('type_name')
    return flask.render_template('page.html')
...
def predict():
    if flask.request.method == "POST":
        ...
        try:
            if hand_type[0] == 'r' or hand_type[0] =='R':
                name = 'right'
            elif hand_type[0] == 'l' or hand_type[0] == 'L':
                name = 'left'
            no_media_show_pointByhand(cv2.cvtColor(np.array(img), cv2.COLOR_BGR2RGB), a, b, c, grid_number = 5, whether_grid = False, hand_type = name)
